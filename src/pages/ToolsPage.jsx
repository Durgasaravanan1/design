        ...
        {TOOLS.map((tool) => {
          ...
          return (
            <Card key={tool.id} className="p-6 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ background: `${tool.color}12`, border: `1.5px solid ${tool.color}25` }}
                  >
                    <Icon size={22} style={{ color: tool.color }} />
                  </div>
                  <div>
              ...
              <div className="flex items-center gap-2 mb-1">
                <UsageBar pct={tool.usage} color={tool.color} />
                <span className="text-xs font-semibold font-mono shrink-0" style={{ color: tool.color }}>
                  {tool.usage}%
                </span>
              </div>
              <p className="text-xs text-gray-400 mb-5">{tool.stat}</p>
              <div className="flex gap-2">
                <button
                  className="flex-1 flex items-center justify-center gap-1.5 rounded-xl h-9 text-sm font-semibold text-white transition-all hover:opacity-90"
                  style={{ background: `linear-gradient(135deg, ${tool.color}, ${tool.color}cc)` }}
                >
                  <Icons.ExternalLink size={13} /> Open Tool
                </button>
                <button className="flex-1 flex items-center justify-center gap-1.5 rounded-xl h-9 text-sm font-medium text-violet-600 bg-violet-50 hover:bg-violet-100 transition-colors">
                  <Icons.BarChart3 size={13} /> Analytics
                </button>
              </div>
            </Card>
          );
